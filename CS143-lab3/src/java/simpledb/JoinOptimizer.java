...
public class JoinOptimizer {
	...
	{
		if(t1pkey && !t2pkey) //if t1 is primary key, then cardinality is card2
			card=card2;
		else if (t2pkey && !t1pkey) //t2 is primary key
			card=card1;
		else //no primary key, the larger of the two
		{
			if(card1>card2)
				card = card1;
			else
	...
	PlanCache cache = new PlanCache();
        ...
        Set<Set<LogicalJoinNode>> joinSet2 = enumerateSubsets(joins, 1);

        int len = joinSet2.size();
        for (int i=1; i <= len; i++){

            Set<Set<LogicalJoinNode>> len2 = enumerateSubsets(joins, i);

            for (Set<LogicalJoinNode> sets : len2)
	    {
                Double bestCost=Double.MAX_VALUE;
                Integer bestCard=Integer.MAX_VALUE;
                Vector<LogicalJoinNode> bestPlan= null;

                for (LogicalJoinNode sets2 : sets)
		{
                    CostCard cc = computeCostAndCardOfSubplan(stats, filterSelectivities, sets2, sets, Double.MAX_VALUE, cache);
                ...
                cache.addPlan(sets, bestCost, bestCard, bestPlan);
            }

        }

        Vector<LogicalJoinNode> returnValue = cache.getOrder(joinSet);
        return returnValue;
    }
